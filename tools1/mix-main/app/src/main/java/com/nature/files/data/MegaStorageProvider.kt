...
/**
 * Persistent simulation for Mega Cloud Storage.
 * Uses a hidden local directory to persist "cloud" data across app restarts.
 */
class MegaStorageProvider(private val context: android.content.Context?) : StorageProvider {
    private val rootDir: File by lazy {
        val dir = File(context?.getExternalFilesDir(null), ".mega_sim")
        if (!dir.exists()) {
            dir.mkdirs()
            // Sample data for first run
            File(dir, "MegaWelcome.txt").writeText("Welcome to simulated Mega Cloud! ☁️🌿")
        }
        dir
    }

    private fun getFileFromPath(path: String): File {
        val relPath = path.removePrefix("mega://root")
    ...
    private fun getUriFromFile(file: File): String {
        ...
        return "mega://root${if (relPath.startsWith("/")) "" else "/"}$relPath"
    }
