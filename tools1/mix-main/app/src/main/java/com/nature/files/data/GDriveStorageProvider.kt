...
/**
 * Persistent simulation for Google Drive Storage.
 * Uses a hidden local directory to persist "cloud" data across app restarts.
 */
class GDriveStorageProvider(private val context: android.content.Context?) : StorageProvider {
    private val rootDir: File by lazy {
        val dir = File(context?.getExternalFilesDir(null), ".gdrive_sim")
        if (!dir.exists()) {
            dir.mkdirs()
            // Sample data for first run
            File(dir, "GDrive_Note.txt").writeText("Welcome to simulated Google Drive! ☁️🌿")
    ...
    private fun getFileFromPath(path: String): File {
        val relPath = path.removePrefix("gdrive://root")
    ...
    private fun getUriFromFile(file: File): String {
        ...
        return "gdrive://root${if (relPath.startsWith("/")) "" else "/"}$relPath"
    }
