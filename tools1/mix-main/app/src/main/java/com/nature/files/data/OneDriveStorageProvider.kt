...
/**
 * Persistent simulation for OneDrive Storage.
 * Uses a hidden local directory to persist "cloud" data across app restarts.
 */
class OneDriveStorageProvider(private val context: android.content.Context?) : StorageProvider {
    private val rootDir: File by lazy {
        val dir = File(context?.getExternalFilesDir(null), ".onedrive_sim")
        if (!dir.exists()) {
            dir.mkdirs()
            // Sample data for first run
            File(dir, "OneDrive_Guide.pdf").writeText("Simulated OneDrive content.")
        }
        dir
    }

    private fun getFileFromPath(path: String): File {
        val relPath = path.removePrefix("onedrive://root")
    ...
    private fun getUriFromFile(file: File): String {
        ...
        return "onedrive://root${if (relPath.startsWith("/")) "" else "/"}$relPath"
    }
