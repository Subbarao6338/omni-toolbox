...
from config_api.models import MetricSource, PromConfig, ConfigString
...
def metric_source_config(request, id=None):
    if request.method == 'POST':
        ...
        instance =MetricSource.objects.create(source_name=source_name,default_config=default_config)
...
def metric_exporter(request):
    print('hello world')
    subprocess.check_call('docker run -d --mount type=bind, source=config/azure.yml,target=/etc/config/azure.yml robustperception/azure_metrics_exporter')
...
def config_view(request,config_name=None):
    if request.method == 'GET':
        ...
        if not obj:
            obj = ConfigString.objects.filter(config_name=config_name).values().first()
