...
def _get_grafana_config():
    ...
    if not obj:
        obj = ConfigString.objects.filter(config_name = 'visualization').values().first()
...
def grafana_alert_rule(request,rule_folder=None,rule_group=None):
    ...
    if request.method == 'GET':
        ...
        url = config['url'] + "/api/ruler/grafana/api/v1/rules/{}/{}?subtype=cortex".format(rule_folder,rule_group)
        response = requests.get(url,headers={'Authorization':'Bearer '+ token})
        return HttpResponse(response)

    if request.method =='POST':
         if (rule_folder ):
            url = config['url'] + "/api/ruler/grafana/api/v1/rules/{}?subtype=cortex".format(rule_folder)
        ...
        for folder in res:
            for group in res[folder]:
                for rule in group["rules"]:
                    ...
                    temp["alert_count"] = 0
                    url_state_history = config['url'] + "/api/annotations?alertId=" + str(rule_id)
                    response_state_history = requests.get(url_state_history, headers={'Authorization':'Bearer '+ token})
