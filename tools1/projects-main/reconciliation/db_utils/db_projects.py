...
def get_active_project_list():
    query = """SELECT project_id,project_name FROM reconcile_schema.projects WHERE isactive=1  ORDER BY project_name"""
    data = execute_get_query(query,)
    return data
...
def delete_project(project_id):
    query="""DELETE projects WHERE reconcile_schema.project_id=%s"""
    print(query)
