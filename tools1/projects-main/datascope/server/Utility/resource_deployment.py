...
def resource_deploy(template_dir, request):
    ...
    if page_type == "kubernetes":
        ...
        autodeploy_utility.update_status(resource_name, "Completed")
        autodeploy_utility.update_provisioned_aks_count(proj_id)
        ...
        if deploymentType == "App Service":
            ...
            autodeploy_utility.updateDeployedResourcesStatus(
                request, json.dumps(resourcedetails), status)
    log.close()
    sys.stdout = stdout
...
def component_deploy(request):
    ...
    print("Kubernetes Name - ", kubernetes_name)
    ...
    print(exit_code, result_dict, logs)
    ...
    # Creating namespace
    subprocess.run(['kubectl', 'create', 'namespace',
                   page_type], stdout=log, stderr=log)

    # set the namespace
    subprocess.run(['kubectl', 'config', 'set-context',
                   kubernetes_name, namespace], stdout=log, stderr=log)

    # Add airflow repo into helm
    subprocess.run(['helm', 'repo', 'add', repo_name, repo_url,
                   '--force-update'], stdout=log, stderr=log)
    ...
    if page_type == "airflow":
        # install airflow
        subprocess.run(['helm', 'install', release_name, chart_name, '--namespace',
                       page_type, '-f', './airflow.yaml', '--debug'], stdout=log, stderr=log)
    elif page_type == "airbyte":
        subprocess.run(['helm', 'install', release_name, chart_name,
                       '--namespace', page_type, '--debug'], stdout=log, stderr=log)
    ...
    if page_type == "airflow":
        ...
        Step2 = "2. Run this Command: \"kubectl port-forward --namespace " + \
            page_type+" service/airflow-webserver 8080:8080\""
        ...
        Step1 = "1. Install KubeCtl"
        Step2 = "2. Run this Command: \"kubectl port-forward --namespace " + \
            page_type+" service/airbyte-webapp 8080:80\""
...
def update_yaml_values(adminuser, adminpassword):
    ...
    with open(os.path.join(cwdPath, filename)) as f:
