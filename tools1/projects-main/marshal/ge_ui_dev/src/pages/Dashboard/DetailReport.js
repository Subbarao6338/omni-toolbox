...
function DetailReport() {
  ...
  return (
    <div>
      ...
      <div className="row pb-2 border-bottom mb-3 ">
        {resultsforDougnut.map((value) => (
          <div
            key={value.dimension}
            style={{
              width: `calc(100% / ${resultsforDougnut.length})`,
              maxWidth: 180,
            }}
          >
      ...
      <div className="row mb-3">
        <DataQualityPieChart dataset={resultsforPie} />
        <div
          className="border col-md-9"
          style={{
...
function DataQualityPieChart({ dataset }) {
  const data = {
  ...
  return (
    <div className="text-center">
      ...
      <div style={{ marginTop: -20 }}>
        {dataset.n_total ? Math.round(dataset.p_success) + "%" : "NA"}
      </div>
    </div>
  );
}
