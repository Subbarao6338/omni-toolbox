...
def mssql_conn_str(USERNAME, PASSWORD, HOST, PORT, DATABASE, DRIVER):
    connection_str = "mssql+pyodbc://" + USERNAME + ":" + PASSWORD + "@" + HOST + ":" + PORT + "/" + DATABASE + "?driver=" + DRIVER + "&charset=utf&autocommit=true"
    return connection_str
...
def get_connection_string(json_body):
    ...
    if database_name == 'postgresql':
        ...
        conn_str = mssql_conn_str(username, password, host, port, database, driver)
        return conn_str
