...
def get_synthetic_data_list():
    ...
    for row in results:
        data_obj = {"Id": row[0], "ModelName": row[1], "ModelType": row[2], "ModelFile": row[3],
                    "RecordCount": row[4], "Status": row[5], "StartTime": row[6], "EndTime": row[7],
                    "train_file": row[8]}
...
def chk_if_data_already_exist(model_selected, row_numbers, data_type):
    connect_engine = connect_sql()
    list_detail = []
    query = """SELECT COUNT(Id) as already_exist FROM Synthetic_Data_Generation WHERE ModelFile='""" + model_selected + """' AND RecordCount='""" + str(
        row_numbers) + """' AND Status='Completed' AND ModelType='""" + data_type + """'; """
    results = connect_engine.execute(query)
    for row in results:
        data = row[0]
    if int(data) > 0:
