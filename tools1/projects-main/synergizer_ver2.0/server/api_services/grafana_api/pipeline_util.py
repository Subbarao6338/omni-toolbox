...
def get_success_pipelines():
    ...
    external_url = "https://management.azure.com/subscriptions/" + subscription_id + "/resourceGroups/" + \
        resource_group + "/providers/Microsoft.DataFactory/factories/" + \
        factory_name + "/pipelines?api-version=2018-06-01"
    token = 'Bearer ' + bearer
    payload = {}
    headers = {
        'Authorization': token
    ...
    response = requests.request(
        "GET", external_url, headers=headers, data=payload)
    ...
    url_runs = "https://management.azure.com/subscriptions/" + subscription_id + "/resourceGroups/" + resource_group + \
        "/providers/Microsoft.DataFactory/factories/" + \
        factory_name + "/queryPipelineRuns?api-version=2018-06-01"
    ...
    for pipeline in pipelines:
        ...
        try:
            response_runs = requests.request(
                "POST", url_runs, headers=headers, data=payload, json=json_body)
            pipelines_runs = response_runs.json()["value"]
            pipelines_runs = [
                run for run in pipelines_runs if run["status"] == "Succeeded"]
            temp = {}
...
def get_failed_pipelines():
    ...
    for pipeline in pipelines:
        ...
        try:
            ...
            pipelines_runs = response_runs.json()["value"]
            pipelines_runs = [
                run for run in pipelines_runs if run["status"] == "Failed"]
            temp = {}
            temp["name"] = pipeline["name"]
