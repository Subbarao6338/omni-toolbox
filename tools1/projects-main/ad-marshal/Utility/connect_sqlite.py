...
def insert_simulator_details(obj):
    try:
        ...
        if get_response == 0:
            ...
            file_obj = (simulator_name, obj_data["simulator_type"], obj_data["data_options"], obj_data["hub_name"],
                        obj_data["device_count"], obj_data["time_delay"], obj_data["simulator_status"], obj_data["telemetry_data_type"])
            sql = """Insert into """ + table_name + \
                """( simulator_name, simulator_type, data_option, hub_name, device_count, time_delay, simulator_status, telemetry_data_type) VALUES (?,?,?,?,?,?,?,?)"""
...
def update_simulator_details(obj):
    try:
        obj_data = json.loads(obj)
        file_obj = (obj_data["simulator_type"], obj_data["data_options"], obj_data["hub_name"], obj_data["device_count"],
                    obj_data["time_delay"], obj_data["simulator_status"], obj_data["telemetry_data_type"], simulator_name)
        sql = """Update """+table_name+""" set simulator_type= ?, data_option = ?, hub_name = ?, device_count = ?, time_delay = ?, simulator_status=?, telemetry_data_type=? where simulator_name = ? """
        object_id = execute_sql(sql, file_obj)
        return object_id
    except Error as e:
        print(e)
...
def get_simulator_details():
    ...
    create_table_sql = 'create table if not exists ' + table_name + ' (simulator_name varchar(50), simulator_type varchar(50), data_option varchar(50), ' \
                                                                    'hub_name varchar(50),device_count integer, time_delay integer, simulator_status varchar(50), telemetry_data_type varchar(50) )'
    conn.execute(create_table_sql)

    select_table_sql = 'select simulator_name,simulator_type,data_option,hub_name,device_count,time_delay,simulator_status,telemetry_data_type from ' + table_name
    for row in cur.execute(select_table_sql):
        data['simulator_name'] = row[0]
        data['simulator_type'] = row[1]
        data['data_options'] = row[2]
        data['hub_name'] = row[3]
        data['device_count'] = row[4]
        data['time_delay'] = row[5]
        data['simulator_status'] = row[6]
        data['telemetry_data_type'] = row[7]
