...
def check_duplicate_datasource(title_name):
    try:
        query = """SELECT COUNT(Title_Name) from [dbo].Anomaly_Input_Files WHERE Title_Name = '""" + title_name + """';"""
...
def save_db_details(connection_str, json_body):
    ...
    dup = check_duplicate_datasource(title_name)
    if(dup == 0):
...
def get_after_remove_file(value):
    ...
    q="""DELETE FROM [dbo].Anomaly_Detection_Form WHERE Anomaly_ID ='""" + value + """';"""
