    ...
    .then((data) => {
      ...
      if(row.status=='completed'){
       ...
       var meta_file_name_str=row.meta_file;
        ...
        html += `<td style="width:5%" class='text-success'>${row.status}</td>`;
        html += `<td style="width:1%"><a href='/download_rel_sdv_file?filetype=new_files&filename=result.zip' download><i class="fa fa-download" aria-hidden="true"></i></a></td>`;
        ...
        html += `<td>--</td>`;
        html += `<td>--</td>`;
        html += `<td class='text-danger'>failed</td>`;
        html += `<td>--</td>`;
        html += `<td style="width:12%">
  ...
  fetch("/get_sdv_models")
    ...
    $("input[name=radio][value='Single']").prop("checked",true).trigger("change");
      ...
      //html += `<td><a href='/download_sdv_file?filetype=sdv_models&filename=${row.sdv_model}' download>${row.sdv_model}</a></td>`;
        html += `<td style="width:12%"> ${row.start_time}</td>`;
        html += `<td>${row.end_time}</td>`;
        html += `<td style="width:5%" class='text-success'>${row.status}</td>`;
        html += `<td style="width:1%"><a title="Download verification result" href='/download_sdv_file?filetype=new_files&filename=new_${row.train_file}' download><i class="fa fa-download" aria-hidden="true"></i></a></td>`;
        html += `<td style="width:12%">
        ...
        html += "<tr>";
        html += `<td> ${index + 1} </td>`;
        html += `<td><a href='/download_sdv_file?filetype=temp_files&filename=${row.train_file}' download>${row.train_file}</a></td>`;
        html += `<td>${row.sdv_model}</td>`;
        html += `<td> ${row.start_time}</td>`;
        html += `<td>--</td>`;
        html += `<td class='text-primary' data-toggle="modal" data-target="#progress_bar" onclick="progress_bar()" nowrap >${row.status}<img class='ms-1' src = '../static/assets/images/info.png' width='10' height='10'></img></td>`;
        html += `<td>--</td>`;
        html += `<td style="width:12%">
                    <button class="btn btn-danger me-2 delete_record" onclick="delete__single_sdv_model('${row.sdv_model}')" title="Delete model"><i class="fa fa-trash" aria-hidden="true"></i></button>
                </td>`;
