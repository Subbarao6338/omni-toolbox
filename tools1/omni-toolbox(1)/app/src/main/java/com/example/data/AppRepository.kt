...
class AppRepository(private val appDao: AppDao) {
    ...
    // --- Contacts ---
    val allContacts: Flow<List<ContactInfo>> = appDao.getAllContacts()
    suspend fun insertContact(contact: ContactInfo) = appDao.insertContact(contact)
    suspend fun updateContact(contact: ContactInfo) = appDao.updateContact(contact)
    suspend fun deleteContactById(id: Int) = appDao.deleteContactById(id)
}
