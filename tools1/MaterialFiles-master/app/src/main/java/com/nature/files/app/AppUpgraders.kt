...
import com.nature.files.compat.PreferenceManagerCompat
import com.nature.files.compat.getDescriptionCompat
import com.nature.files.compat.readBooleanCompat
import com.nature.files.compat.writeBooleanCompat
import com.nature.files.compat.writeParcelableListCompat
import com.nature.files.file.DocumentTreeUri
import com.nature.files.file.asExternalStorageUriOrNull
import com.nature.files.file.displayName
import com.nature.files.file.storageVolume
import com.nature.files.filelist.FileSortOptions
import com.nature.files.navigation.BookmarkDirectory
import com.nature.files.navigation.StandardDirectorySettings
import com.nature.files.provider.archive.ArchiveFileSystem
import com.nature.files.provider.common.ByteString
import com.nature.files.provider.common.moveToByteString
import com.nature.files.provider.content.ContentFileSystem
import com.nature.files.provider.document.DocumentFileSystem
import com.nature.files.provider.document.resolver.ExternalStorageProviderHacks
import com.nature.files.provider.linux.LinuxFileSystem
import com.nature.files.provider.root.RootStrategy
import com.nature.files.provider.sftp.SftpFileSystem
import com.nature.files.provider.smb.SmbFileSystem
import com.nature.files.storage.DocumentTree
import com.nature.files.storage.FileSystemRoot
import com.nature.files.storage.PrimaryStorageVolume
import com.nature.files.util.StableUriParceler
import com.nature.files.util.asBase64
import com.nature.files.util.readParcelable
import com.nature.files.util.readParcelableListCompat
import com.nature.files.util.toBase64
import com.nature.files.util.toByteArray
import com.nature.files.util.use
        ...
        Parcel.obtain().use { newParcel ->
            ...
            Parcel.obtain().use { oldParcel ->
                ...
                repeat(size) {
                    ...
                    oldParcel.readInt()
                    val className = oldParcel.readString()
                    if (className == "com.nature.files.storage.SmbServer") {
                        newParcel.writeInt(PARCEL_VAL_PARCELABLE)
                        newParcel.writeString("com.nature.files.storage.SmbServer")
                        ...
                        newParcel.writeString(
                            "com.nature.files.provider.smb.client.Authority"
                        )
                        ...
                        oldParcel.readString()
                        newParcel.writeString(
                            "com.nature.files.provider.smb.client.Authentication"
                        )
...
private fun migratePath1_4_0(oldParcel: Parcel, newParcel: Parcel) {
    ...
    when (className) {
        ...
        "com.nature.files.provider.sftp.SftpPath" ->
            newParcel.writeParcelable(oldParcel.readParcelable<SftpFileSystem>(), 0)
        "com.nature.files.provider.smb.SmbPath" ->
            ...
            Parcel.obtain().use { oldParcel ->
                ...
                repeat(size) {
                    ...
                    when (oldParcel.readString()) {
                        "com.nature.files.storage.SftpServer" -> {
                            newParcel.writeInt(PARCEL_VAL_PARCELABLE)
                            newParcel.writeString("com.nature.files.storage.SftpServer")
                            val id = oldParcel.readLong()
                            newParcel.writeLong(id)
                            val customName = oldParcel.readString()
                            newParcel.writeString(customName)
                ...
                repeat(size) {
                    ...
                    when (oldParcel.readString()) {
                        "com.nature.files.storage.SftpServer" -> {
                            ...
                            val authenticationPasswordOrPrivateKey = oldParcel.readString()
                            newParcel.writeString(authenticationPasswordOrPrivateKey)
                            if (authenticationClassName == "com.nature.files.provider.sftp"
                ...
                Parcel.obtain().use { oldParcel ->
                    ...
                    readWriteLengthPrefixedValue(oldParcel, newParcel) {
                        ...
                        repeat(size) {
                            ...
                            oldParcel.setDataPosition(oldPosition)
                            when (className) {
                                "com.nature.files.storage.DocumentManagerShortcut" -> {
                                    newParcel.writeInt(oldParcel.readInt())
                                    readWriteLengthPrefixedValue(oldParcel, newParcel) {
                                        oldParcel.readString()
                                        newParcel.writeString(
                                            "com.nature.files.storage" +
