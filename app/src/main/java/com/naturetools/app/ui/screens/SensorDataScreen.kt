...
fun SensorDataScreen(navController: NavHostController) {
    ...
    ToolScreen(title = "Sensor Data", onBack = { navController.popBackStack() }) { padding ->
            ...
            items(sensorList) { sensor ->
                Card(modifier = Modifier.fillMaxWidth()) {
                    Column(modifier = Modifier.padding(16.dp)) {
                        ...
                        Text("Type: ${sensor.stringType}", style = MaterialTheme.typography.bodySmall, color = MaterialTheme.colorScheme.primary)
                        Text("Power: ${sensor.power} mA", style = MaterialTheme.typography.bodySmall)
                        Text("Max Range: ${sensor.maximumRange}", style = MaterialTheme.typography.bodySmall)
                    }
                }
            }
        }
